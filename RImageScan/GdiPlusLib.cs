...
namespace RImageScan.GdiPlusLib
{
...
public class Gdip
    ...
    public static bool SaveImage(string picname, IntPtr bminfo, IntPtr pixdat, string TempDir)
    {
        string TempDirConf = string.Empty;
        string ImageName = string.Empty;
        IntPtr img;
        try
        {
            TempDirConf = System.Configuration.ConfigurationSettings.AppSettings["DIRIMGSCANNER"].ToString() + "\\" + TempDir;
            ImageName = TempDirConf + "\\" + picname;

            if (!System.IO.Directory.Exists(TempDirConf)) System.IO.Directory.CreateDirectory(TempDirConf);
            Guid clsid;

            if (!GetCodecClsid(ImageName, out clsid))
            {
                MessageBox.Show("Formato de extensão da Image irreconhecível. " + Path.GetExtension(ImageName),
                                "Image Codec", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return false;
            }
            ...
            st = GdipSaveImageToFile(img, ImageName, ref clsid, IntPtr.Zero);
            GdipDisposeImage(img);
            return st == 0;
